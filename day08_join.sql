...
/*============================== JOIN İSLEMLERİ ===============================
    ...
    2) LEFT JOIN:  Ilk tabloda (Sol) olun sonuclari gosterir
    3) RIGHT JOIN: Sadece Ikinci(sağ) tabloda olan  sonuclari gosterir
...
------------------------------------------------------------------------------*/
    ...
    select sir.sirket_id, sir.sirket_isim, sip.siparis_id, sip.siparis_tarihi
...
-- join lerde bağlama işi 'ON' ile yapılır
...
/* -----------------------------------------------------------------------------
  ...
  maaslarini ve   maas siralı olarak (Çoktan aza) listeleyiniz.

  
  ayrica MUDUR isimlerini 
...
select b.bolum_isim,p.personel_isim,p.maas,p.meslek
from bolumler  b
...
select b.bolum_isim,p.personel_isim,p.maas,p.meslek,(select personel_isim from personel where p.mudur_id=personel_id) as müdürü
from bolumler  b
right join personel  p
on p. bolum_id=b.bolum_id
where p.mudur_id in(1222,1333)
