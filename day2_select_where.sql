...
/* ======================= SELECT - LIKE ======================================
    ...
    PATTERN İÇİN
    -------------
    %    ---> 0 veya daha fazla karakteri belirtir.  harf sayısı belli olmayan
...
/*-------------------------------------------------------
 örnek 9 ismi A harfi ile başlayanları listeleyin
 --------------------------------------------------------*/
 
 select * from personel
 where isim like 'a%'; -- a ile başlasın % sonrası sınırsız harf anlamına gelir
 
 /* -----------------------------------------------------------------------------
  ORNEK10:  ismi n harfi ile bitenleri listeleyiniz
 -----------------------------------------------------------------------------*/
    SELECT * FROM personel
    WHERE isim LIKE '%n';--  % baş harf lerinin karekter sayısını bilmediğimizden sonsuz anlamında
...
/* ========================================================================== */
    ...
    INSERT INTO kelimeler VALUES (1015, 'hooooooooooooot', 5);
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
    ...
    where REGEXP_LIKE(kelime,'at|ot','c'); -- 'c' harfi sadece küçük harf demek
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
    select * from kelimeler
    where REGEXP_LIKE(kelime,'at|ot'); -- c olmassa büyük küçük bakmadan kelimelerden getirr
 ...
 -----------------------------------------------------------------------------*/   
    ...
    select * from kelimeler
    where REGEXP_LIKE(kelime,'^ho|^hi');-- ^ kelimelerin başalangacını gösterir
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/   
     ...
     select * from kelimeler
    where REGEXP_LIKE(kelime,'t$|m$');-- $ cümleyada harfin sonuna ekleyerek yaparak sonda olanı ararız
...
/* -----------------------------------------------------------------------------
 ...
 SELECT * FROM kelimeler
    WHERE REGEXP_LIKE(kelime, 'h[a|i]t'); -- [] içinde | olmasına gerek yok
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/     
   ...
   SELECT * FROM kelimeler
    WHERE REGEXP_LIKE(kelime, 'm|i|e');--  veya [mie] de olur [] veya anlamınada gelir
...
SELECT * FROM kelimeler
    WHERE REGEXP_LIKE(kelime, '^a|^s');-- '^[as]'
...
-----------------------------------------------------------------------------*/ 
    SELECT * FROM kelimeler
    WHERE REGEXP_LIKE(kelime, '[o]{3}');-- {} en az kaçtane olmasını istediğimizde yazarız
