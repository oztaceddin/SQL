...
-------- Örnek 1: Hangi isimden kaç tane olduğunu bulun.
 ...
 SELECT 
    ograd, COUNT(ograd) AS isim_adedi
FROM
    ogrenci
GROUP BY ograd
ORDER BY ograd;
...
select sinif, count(ogrno)ogrenci_sayisi from ogrenci
group by sinif;
...
select sinif,cinsiyet, count(cinsiyet) kişi_sayısı
from ogrenci
...
select turno,count(idno) adet from kitap
group by turno;
...
select turno,max(sayfasayisi) sayfa_sayısı from kitap
group by turno
order by turno desc;
...
select turno,min(sayfasayisi) sayfa_sayısı from kitap
group by turno
order by turno ;
